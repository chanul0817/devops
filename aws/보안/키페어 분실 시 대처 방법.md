...
- Amazon Linux는 보통 `/home/ec2-user/.ssh/authorized_keys`, Ubuntu는 `/home/ubuntu/.ssh/authorized_keys`를 봄
- `>`로 덮어쓰면 기존 키가 날아갈 수 있어서, 실무에서는 새 공개키를 한 줄 추가하는 쪽이 더 안전함
- SELinux 쓰는 환경이면 권한만 맞아도 접속이 안 될 수 있어서 `.ssh` 권한까지 같이 확인하는 편이 좋음
...
### 방법 3. SSM Session Manager로 우회 접속

- 인스턴스에 SSM Agent가 있고 IAM 권한도 붙어 있으면, 키페어 없이도 콘솔에서 셸 접속 가능
- 이 경우에는 바로 들어가서 `authorized_keys`를 수정하거나, 아예 키 기반 SSH 의존도를 줄이는 쪽으로 운영하기도 함
- 단, private subnet에 있더라도 SSM 통신 경로(NAT 또는 VPC Endpoint)는 살아 있어야 함
