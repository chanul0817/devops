...
- 파일이 안 보이면 prefix를 잘못 본 경우도 있음
- S3 작업은 작은 변경이라도 영향 범위를 먼저 생각하고 진행
- 동기화 작업은 삭제되는 파일 목록을 먼저 보는 게 안전함

### S3 작업 - 재현할 때

- 삭제 옵션이 들어간 명령은 먼저 dryrun으로 확인
- S3 이슈는 한 번 더 재현되는지 확인해야 임시 현상과 구분됨
- 운영 버킷 작업은 명령과 대상 경로를 한 번 더 확인

### S3 작업 - 로그 남길 때

- 버킷 권한 문제는 IAM과 bucket policy를 같이 봄
- S3 확인 결과는 다음 사람이 이어 볼 수 있게 짧게라도 기록
- 권한 오류와 경로 오류를 나눠서 확인

### S3 작업 - 원인 좁히기

- 파일이 안 보이면 prefix를 잘못 본 경우도 있음
- S3 원인은 네트워크, 권한, 설정, 리소스로 나눠 보면 덜 헤맴
- 동기화 작업은 삭제되는 파일 목록을 먼저 보는 게 안전함

### S3 작업 - 반복 장애

- 삭제 옵션이 들어간 명령은 먼저 dryrun으로 확인
- S3 문제가 반복되면 자동 복구보다 왜 반복되는지 먼저 확인
- 운영 버킷 작업은 명령과 대상 경로를 한 번 더 확인
