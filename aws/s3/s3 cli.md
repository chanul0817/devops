...
- `--delete`는 로컬에 없는 파일을 S3에서도 지움
- 정적 사이트 배포에는 편하지만, 실수하면 필요한 파일도 같이 날아갈 수 있음
- 처음에는 `--dryrun`으로 결과를 보고 실행하는 게 안전함

### S3 작업 - 처음 볼 때

- 삭제 옵션이 들어간 명령은 먼저 dryrun으로 확인
- S3 문제는 처음부터 결론내기보다 증상을 먼저 작게 나누는 게 좋음
- 운영 버킷 작업은 명령과 대상 경로를 한 번 더 확인

### S3 작업 - 배포 직후

- 버킷 권한 문제는 IAM과 bucket policy를 같이 봄
- 배포 직후 S3 쪽 지표가 흔들리면 변경 범위부터 다시 확인
- 권한 오류와 경로 오류를 나눠서 확인

### S3 작업 - 장애 시간대

- 파일이 안 보이면 prefix를 잘못 본 경우도 있음
- S3 확인은 장애가 시작된 시간과 마지막 정상 시간을 같이 잡고 봄
- 동기화 작업은 삭제되는 파일 목록을 먼저 보는 게 안전함

### S3 작업 - 권한 볼 때

- 삭제 옵션이 들어간 명령은 먼저 dryrun으로 확인
- S3 문제가 권한처럼 보여도 경로, 대상, 실행 주체를 같이 확인
- 운영 버킷 작업은 명령과 대상 경로를 한 번 더 확인

### S3 작업 - 되돌리기 전

- 버킷 권한 문제는 IAM과 bucket policy를 같이 봄
- S3을 되돌리기 전에는 현재 상태를 남겨둬야 비교가 쉬움
- 권한 오류와 경로 오류를 나눠서 확인
