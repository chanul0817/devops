...
- 인스턴스가 켜져 있어도 보안 그룹이 막혀 있으면 접속이 안 됨
- SSH는 22번, HTTP는 80번, HTTPS는 443번을 먼저 확인
- 내 IP가 바뀌면 기존에 열어둔 SSH 규칙이 안 맞을 수 있음

### user data 확인

- 초기 설정 자동화는 user data로 처리할 수 있음
- 스크립트가 실패하면 인스턴스는 떠도 앱은 준비되지 않을 수 있음
- `/var/log/cloud-init-output.log`에서 실행 결과를 확인

### EC2 점검 - 처음 볼 때

- 앱이 안 뜨면 user data와 system log도 같이 확인
- EC2 문제는 처음부터 결론내기보다 증상을 먼저 작게 나누는 게 좋음
- 권한 문제인지 네트워크 문제인지 먼저 갈라서 봄

### EC2 점검 - 배포 직후

- 재시작 뒤 public IP가 바뀌었는지 확인할 때가 있음
- 배포 직후 EC2 쪽 지표가 흔들리면 변경 범위부터 다시 확인
- 디스크, 메모리, 네트워크 지표를 같이 보면 원인이 좁혀짐

### EC2 점검 - 장애 시간대

- 접속 문제는 인스턴스 상태와 보안 그룹을 먼저 나눠봄
- EC2 확인은 장애가 시작된 시간과 마지막 정상 시간을 같이 잡고 봄
- 운영 서버는 변경 전후 상태를 짧게라도 기록

### EC2 점검 - 권한 볼 때

- 앱이 안 뜨면 user data와 system log도 같이 확인
- EC2 문제가 권한처럼 보여도 경로, 대상, 실행 주체를 같이 확인
- 권한 문제인지 네트워크 문제인지 먼저 갈라서 봄

### EC2 점검 - 되돌리기 전

- 재시작 뒤 public IP가 바뀌었는지 확인할 때가 있음
- EC2을 되돌리기 전에는 현재 상태를 남겨둬야 비교가 쉬움
- 디스크, 메모리, 네트워크 지표를 같이 보면 원인이 좁혀짐

### EC2 점검 - 운영에서 조심할 점

- 접속 문제는 인스턴스 상태와 보안 그룹을 먼저 나눠봄
- EC2 작업은 작은 변경이라도 영향 범위를 먼저 생각하고 진행
- 운영 서버는 변경 전후 상태를 짧게라도 기록

### EC2 점검 - 재현할 때

- 앱이 안 뜨면 user data와 system log도 같이 확인
- EC2 이슈는 한 번 더 재현되는지 확인해야 임시 현상과 구분됨
- 권한 문제인지 네트워크 문제인지 먼저 갈라서 봄

### EC2 점검 - 로그 남길 때

- 재시작 뒤 public IP가 바뀌었는지 확인할 때가 있음
- EC2 확인 결과는 다음 사람이 이어 볼 수 있게 짧게라도 기록
- 디스크, 메모리, 네트워크 지표를 같이 보면 원인이 좁혀짐

### EC2 점검 - 원인 좁히기

- 접속 문제는 인스턴스 상태와 보안 그룹을 먼저 나눠봄
- EC2 원인은 네트워크, 권한, 설정, 리소스로 나눠 보면 덜 헤맴
- 운영 서버는 변경 전후 상태를 짧게라도 기록

### EC2 점검 - 반복 장애

- 앱이 안 뜨면 user data와 system log도 같이 확인
- EC2 문제가 반복되면 자동 복구보다 왜 반복되는지 먼저 확인
- 권한 문제인지 네트워크 문제인지 먼저 갈라서 봄
