...
- EBS 볼륨은 같은 AZ의 EC2에 붙이는 구조
- 다른 AZ 인스턴스에 바로 붙이려면 스냅샷을 떠서 새 볼륨을 만드는 식으로 처리
- 장애 대비를 생각하면 스냅샷 주기와 복구 시간을 같이 봐야 함

### EBS 늘린 뒤

- 콘솔에서 볼륨 크기를 늘린 것만으로 파일시스템이 바로 커지지는 않을 수 있음
- OS 안에서 파티션과 파일시스템 확장까지 확인
- 작업 전 스냅샷을 떠두면 되돌릴 여지가 생김
