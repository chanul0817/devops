...
### NAT Gateway는 비용도 같이 보기

- NAT Gateway는 시간 비용과 처리량 비용이 같이 붙음
- 개발 환경에서 계속 켜두면 생각보다 비용이 빨리 쌓일 수 있음
- private subnet이 정말 외부로 나가야 하는지 먼저 확인

### private subnet에서 외부로 나갈 때

- 인터넷에서 직접 들어오지는 못하게 하고, 나가는 통신만 NAT로 처리할 수 있음
- 패키지 설치나 외부 API 호출이 필요하면 route table을 확인
- NAT가 없으면 보안 업데이트도 막힐 수 있음
