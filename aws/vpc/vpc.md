...
- Subnet이 어떤 Route Table에 연결되어 있는지 먼저 봄
- 인터넷 통신은 `0.0.0.0/0`이 IGW나 NAT 쪽으로 나가는지 확인
- 보안 그룹이 열려 있어도 라우팅이 없으면 통신이 안 됨

### VPC 통신 - 처음 볼 때

- private subnet은 외부로 나가는 경로가 따로 필요함
- VPC 문제는 처음부터 결론내기보다 증상을 먼저 작게 나누는 게 좋음
- 보안 규칙이 열려 있어도 라우팅이 없으면 통신이 안 됨

### VPC 통신 - 배포 직후

- 통신이 안 되면 라우팅, 보안 그룹, NACL 순서로 나눠봄
- 배포 직후 VPC 쪽 지표가 흔들리면 변경 범위부터 다시 확인
- 대상 IP보다 경로가 맞는지 먼저 보는 게 빠름

### VPC 통신 - 장애 시간대

- 서브넷이 어느 Route Table을 보는지 먼저 확인
- VPC 확인은 장애가 시작된 시간과 마지막 정상 시간을 같이 잡고 봄
- 같은 VPC 안과 외부에서 테스트 결과가 다를 수 있음

### VPC 통신 - 권한 볼 때

- private subnet은 외부로 나가는 경로가 따로 필요함
- VPC 문제가 권한처럼 보여도 경로, 대상, 실행 주체를 같이 확인
- 보안 규칙이 열려 있어도 라우팅이 없으면 통신이 안 됨

### VPC 통신 - 되돌리기 전

- 통신이 안 되면 라우팅, 보안 그룹, NACL 순서로 나눠봄
- VPC을 되돌리기 전에는 현재 상태를 남겨둬야 비교가 쉬움
- 대상 IP보다 경로가 맞는지 먼저 보는 게 빠름

### VPC 통신 - 운영에서 조심할 점

- 서브넷이 어느 Route Table을 보는지 먼저 확인
- VPC 작업은 작은 변경이라도 영향 범위를 먼저 생각하고 진행
- 같은 VPC 안과 외부에서 테스트 결과가 다를 수 있음

### VPC 통신 - 재현할 때

- private subnet은 외부로 나가는 경로가 따로 필요함
- VPC 이슈는 한 번 더 재현되는지 확인해야 임시 현상과 구분됨
- 보안 규칙이 열려 있어도 라우팅이 없으면 통신이 안 됨

### VPC 통신 - 로그 남길 때

- 통신이 안 되면 라우팅, 보안 그룹, NACL 순서로 나눠봄
- VPC 확인 결과는 다음 사람이 이어 볼 수 있게 짧게라도 기록
- 대상 IP보다 경로가 맞는지 먼저 보는 게 빠름

### VPC 통신 - 원인 좁히기

- 서브넷이 어느 Route Table을 보는지 먼저 확인
- VPC 원인은 네트워크, 권한, 설정, 리소스로 나눠 보면 덜 헤맴
- 같은 VPC 안과 외부에서 테스트 결과가 다를 수 있음

### VPC 통신 - 반복 장애

- private subnet은 외부로 나가는 경로가 따로 필요함
- VPC 문제가 반복되면 자동 복구보다 왜 반복되는지 먼저 확인
- 보안 규칙이 열려 있어도 라우팅이 없으면 통신이 안 됨
