...
- 여러 서비스가 같이 뜰 때는 의존 서비스 상태를 먼저 봄
- compose 문제는 처음부터 결론내기보다 증상을 먼저 작게 나누는 게 좋음
- volume을 쓰는 서비스는 데이터 경로를 먼저 확인

### compose 운영 - 배포 직후

- 컨테이너가 살아 있어도 앱 준비가 끝났는지는 따로 확인
- 배포 직후 compose 쪽 지표가 흔들리면 변경 범위부터 다시 확인
- logs, ps, events를 같이 보면 흐름을 잡기 쉬움

### compose 운영 - 장애 시간대

- 환경 변수 변경 뒤에는 재생성 여부를 같이 봄
- compose 확인은 장애가 시작된 시간과 마지막 정상 시간을 같이 잡고 봄
- 재시작 정책이 문제를 숨기고 있지 않은지도 봐야 함

### compose 운영 - 권한 볼 때

- 여러 서비스가 같이 뜰 때는 의존 서비스 상태를 먼저 봄
- compose 문제가 권한처럼 보여도 경로, 대상, 실행 주체를 같이 확인
- volume을 쓰는 서비스는 데이터 경로를 먼저 확인

### compose 운영 - 되돌리기 전

- 컨테이너가 살아 있어도 앱 준비가 끝났는지는 따로 확인
- compose을 되돌리기 전에는 현재 상태를 남겨둬야 비교가 쉬움
- logs, ps, events를 같이 보면 흐름을 잡기 쉬움

### compose 운영 - 운영에서 조심할 점

- 환경 변수 변경 뒤에는 재생성 여부를 같이 봄
- compose 작업은 작은 변경이라도 영향 범위를 먼저 생각하고 진행
- 재시작 정책이 문제를 숨기고 있지 않은지도 봐야 함
...
- 환경 변수 변경 뒤에는 재생성 여부를 같이 봄
- compose 원인은 네트워크, 권한, 설정, 리소스로 나눠 보면 덜 헤맴
- 재시작 정책이 문제를 숨기고 있지 않은지도 봐야 함

### compose 운영 - 반복 장애

- 여러 서비스가 같이 뜰 때는 의존 서비스 상태를 먼저 봄
- compose 문제가 반복되면 자동 복구보다 왜 반복되는지 먼저 확인
- volume을 쓰는 서비스는 데이터 경로를 먼저 확인

### compose 운영 - 체크리스트

- 컨테이너가 살아 있어도 앱 준비가 끝났는지는 따로 확인
- compose 체크리스트는 실제 장애 때 바로 칠 수 있을 정도로 짧게 유지
- logs, ps, events를 같이 보면 흐름을 잡기 쉬움
