...
- 연결이 필요한 서비스는 대부분 TCP 기준으로 먼저 확인
- UDP는 연결 상태가 없어서 단순 포트 오픈 확인만으로 판단하기 어려움
- 방화벽, 라우팅, 애플리케이션 로그를 같이 봐야 함
...
- 연결 실패가 앱 문제인지 네트워크 문제인지 먼저 나눔
- 방화벽이 중간에서 막는 경우 timeout으로 보일 때가 많음
- 같은 VPC 안과 밖에서 결과가 다를 수 있음
