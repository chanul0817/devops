...
- 원래 환경이 정말 정상인지 먼저 확인
- 데이터가 어느 쪽이 최신인지 확인하지 않으면 복구 후 더 꼬일 수 있음
- 전환 절차와 되돌림 절차를 같이 적어두는 게 좋음

### DNS 기반 전환

- TTL이 길면 장애 전환 후에도 예전 주소를 보는 사용자가 남을 수 있음
- 전환 전에 TTL을 낮춰두는 전략을 쓰기도 함
- 전환 완료 후에는 모니터링으로 실제 트래픽 위치를 확인
