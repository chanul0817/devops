...
- 브랜치 확인: `git branch`
- 원격 브랜치까지 확인: `git branch -r`
- 로컬/원격 추적 정보 확인: `git branch -vv`
...
- PR 머지 후에는 사용한 작업 브랜치를 정리해두면 브랜치 목록이 덜 헷갈림.
...
`-d`는 병합된 브랜치만 삭제하고, `-D`는 강제로 삭제함.
실수 방지를 위해 보통은 `-d`를 먼저 쓰는 게 좋음.
...
- 머지된 브랜치는 남겨둘 이유가 없으면 정리
- 원격 브랜치 삭제는 팀에서 쓰는 브랜치인지 확인하고 진행
