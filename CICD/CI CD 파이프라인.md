...
- cache는 의존성 다운로드 시간을 줄이기 위해 사용
- artifact는 빌드 결과물을 다음 단계로 넘길 때 사용
- 둘 다 비슷해 보이지만 목적이 달라서 구분해서 쓰는 게 좋음

### 빠르게 실패시키기

- lint, unit test처럼 빠른 검사를 앞에 두면 피드백이 빨라짐
- 오래 걸리는 빌드나 배포는 앞 단계가 통과한 뒤 실행
- 실패 원인이 로그에 바로 보이도록 단계 이름을 명확히 둠

### CI에서 secret 다루기

- 토큰은 저장소 파일에 두지 말고 CI secret 기능을 사용
- 로그에 secret이 찍히지 않게 echo나 debug 옵션을 조심
- 권한은 필요한 범위로만 좁혀두는 게 안전함

### CI 확인 - 처음 볼 때

- 빌드 실패는 의존성, 환경 변수, 캐시 순서로 많이 봄
- CI 문제는 처음부터 결론내기보다 증상을 먼저 작게 나누는 게 좋음
- 로그에 secret이 찍히지 않는지 확인

### CI 확인 - 배포 직후

- 배포 단계는 앞 단계가 통과한 뒤에만 실행되게 둠
- 배포 직후 CI 쪽 지표가 흔들리면 변경 범위부터 다시 확인
- 캐시 문제처럼 보이면 깨끗한 빌드도 한 번 확인

### CI 확인 - 장애 시간대

- 빠른 검사를 앞에 두면 실패를 빨리 알 수 있음
- CI 확인은 장애가 시작된 시간과 마지막 정상 시간을 같이 잡고 봄
- 실패한 단계 이름이 명확해야 다음 확인이 쉬움

### CI 확인 - 권한 볼 때

- 빌드 실패는 의존성, 환경 변수, 캐시 순서로 많이 봄
- CI 문제가 권한처럼 보여도 경로, 대상, 실행 주체를 같이 확인
- 로그에 secret이 찍히지 않는지 확인

### CI 확인 - 되돌리기 전

- 배포 단계는 앞 단계가 통과한 뒤에만 실행되게 둠
- CI을 되돌리기 전에는 현재 상태를 남겨둬야 비교가 쉬움
- 캐시 문제처럼 보이면 깨끗한 빌드도 한 번 확인

### CI 확인 - 운영에서 조심할 점

- 빠른 검사를 앞에 두면 실패를 빨리 알 수 있음
- CI 작업은 작은 변경이라도 영향 범위를 먼저 생각하고 진행
- 실패한 단계 이름이 명확해야 다음 확인이 쉬움

### CI 확인 - 재현할 때

- 빌드 실패는 의존성, 환경 변수, 캐시 순서로 많이 봄
- CI 이슈는 한 번 더 재현되는지 확인해야 임시 현상과 구분됨
- 로그에 secret이 찍히지 않는지 확인

### CI 확인 - 로그 남길 때

- 배포 단계는 앞 단계가 통과한 뒤에만 실행되게 둠
- CI 확인 결과는 다음 사람이 이어 볼 수 있게 짧게라도 기록
- 캐시 문제처럼 보이면 깨끗한 빌드도 한 번 확인

### CI 확인 - 원인 좁히기

- 빠른 검사를 앞에 두면 실패를 빨리 알 수 있음
- CI 원인은 네트워크, 권한, 설정, 리소스로 나눠 보면 덜 헤맴
- 실패한 단계 이름이 명확해야 다음 확인이 쉬움

### CI 확인 - 반복 장애

- 빌드 실패는 의존성, 환경 변수, 캐시 순서로 많이 봄
- CI 문제가 반복되면 자동 복구보다 왜 반복되는지 먼저 확인
- 로그에 secret이 찍히지 않는지 확인
